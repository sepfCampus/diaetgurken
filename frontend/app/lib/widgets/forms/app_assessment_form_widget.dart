...
import 'package:app/widgets/forms/app_assessment_input_wrapper.dart';
...
class _AppAssessmentFormWidgetState extends State<AppAssessmentFormWidget>
{
  ...
  void _initializeFields()
  {
    ...
    {
      final existingElement = widget.assessment.elements[element.name];
      ...
      if(element is FormDateElement)
      {
        ...
        _controllers[element.name] = TextEditingController(
          text: value == null ? '' : value.toIso8601String().split('T').first,
        );
      ...
      {
        final selectedOptions = element.options.values.where((option)
        {
        ...
        if(existingElement == null)
        {
          ...
          {
            widget.assessment.setValue(element.name, selectedOptions);
          }

          else
          {
            widget.assessment.setValue(element.name, selectedOptions.isNotEmpty ? selectedOptions.first : null);
          }
        }
      }
    }
  ...
  bool _noAnswerProvided(String fieldName)
  {
    return widget.assessment.elements[fieldName]?.noAnswerProvided ?? false;
  }
  ...
  void _setValue(String fieldName, dynamic value)
  {
    setState(()
    {
      widget.assessment.setValue(fieldName, value);

      final element = widget.assessment.elements[fieldName];

      if(element != null)
      {
        element.noAnswerProvided = false;
      }
    });
  ...
  void _setNoAnswer(String fieldName)
  {
    setState(()
    {
      final element = widget.assessment.elements[fieldName];
      ...
      if(element == null)
      {
        widget.assessment.elements[fieldName] = AssessmentElement(fieldName, null, true);
      }

      else
      {
        element.noAnswerProvided = !element.noAnswerProvided;
      }
    });
  ...
  Widget _wrapAssessmentField({ required String fieldName, required Widget child })
  {
    return AppAssessmentInputWrapper(
      noAnswerProvided: _noAnswerProvided(fieldName),
      onNoAnswerPressed: ()
      {
        _setNoAnswer(fieldName);
      },
      child: child,
    );
  }
  ...
  {
    final elements = widget.elements.where((element)
    {
  ...
  Widget _buildField(FormBaseElement element)
  {
    ...
    if(element is FormTrueFalseElement)
    {
      return _buildTrueFalseSelection(element);
    }
  ...
  Widget _buildTrueFalseSelection(FormTrueFalseElement element)
  {
    return _wrapAssessmentField(
      fieldName: element.name,
  ...
  Widget _buildSingleSelection(FormSelectionElement element)
  {
    return _wrapAssessmentField(
      ...
      child: AppRadioGroup<String>(
        ...
        labelBuilder: (optionName)
        {
          return element.options[optionName]?.text ?? optionName;
        },
        onChanged: (value)
        {
          _setValue(element.name, value);
  ...
  Widget _buildMultipleSelection(FormSelectionElement element)
  {
    ...
    return _wrapAssessmentField(
      fieldName: element.name,
      child: AppMultipleSelection<String>(
        title: element.displayName ?? element.name,
        options: element.options.keys.toList(),
